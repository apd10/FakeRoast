...
class FedOrchestrator:
    ...
    def merge_wt_normalize(models, is_global):
        ...
        for i in range(len(models)):
            ...
            for n, m in model.named_modules():
                if type(m) in [FakeRoastLinear, FakeRoastConv2d] :
                    dics[i][n+'wt'] = m.wt_comp_to_orig(m.weight.data)
        ...
        for k in dics[0].keys():
            ...
            final_dic[k] = final_dic[k] / len(dic)
        ...
        for i in range(len(models)):
            ...
            for n, m in model.named_modules():
                if type(m) in [FakeRoastLinear, FakeRoastConv2d, FakeRoastEmbedding] :
                    ...
                    if is_global:
                        ...
                        m.weight.data = alpha * m.weight.data + (1 - alpha) * torch.div(wt, ct+1e-3)
            ...
            if is_global:
                for n, m in model.named_modules():
                    if type(m) in [FakeRoastLinear, FakeRoastConv2d, FakeRoastEmbedding] :
                        m.weight.data = alpha * m.weight.data + (1 - alpha) * torch.div(global_wt, global_ct+1e-3)
                        # update only once
                        break
